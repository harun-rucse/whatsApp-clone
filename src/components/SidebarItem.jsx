...
function SidebarItem({ contact }) {
  const { handleSelectContact } = useApp();
  const { image, userName, lastMessage, time } = contact;

  return (
    <div
      className="flex items-center gap-4 px-5 pt-1 transition hover:bg-[#202c33] cursor-pointer"
